#!/usr/bin/env python3
"""Render 20 visual mockups of the 20×16 grid as full grids with highlighted letters."""
...
# bright white on black for active, dark gray for inactive
ON  = "\033[1;97;40m"
OFF = "\033[2;90;40m"
RST = "\033[0m"
BG  = "\033[40m"
...
def render_grid(h, m):
    ...
    for ri, row in enumerate(GRID):
        ...
        for ci, ch in enumerate(row):
            if (ri, ci) in active:
                cells.append(f"{ON} {ch} {RST}{BG}")
            else:
                cells.append(f"{OFF} {ch} {RST}{BG}")
        lines.append(f"{BG} " + "".join(cells) + f" {RST}")
    return "\n".join(lines)
...
for i, (h, m) in enumerate(SAMPLES, 1):
    d = describe(h, m)
    print(f"\n  \033[1;33m#{i}  {h}:{m:02d}\033[0m  →  \033[1;36m{d}\033[0m\n")
    print(render_grid(h, m))
    print()
