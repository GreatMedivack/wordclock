#!/usr/bin/env python3
"""Render 16×17 word clock — И on row 16, МИНУТА/2/3/4/МИНУТЫ on row 17."""
...
W_PLUS    = (16,  0,  1)   # +
W_MINUTA  = (16,  1,  7)   # МИНУТА
W_2       = (16,  7,  8)   # 2
W_3       = (16,  8,  9)   # 3
W_4       = (16,  9, 10)   # 4
W_MINUTY  = (16, 10, 16)   # МИНУТЫ
...
def active_positions(hour12, minute):
    ...
    if extra == 1:
        add_word(active, W_PLUS)
        add_word(active, W_MINUTA)
    elif extra == 2:
        add_word(active, W_PLUS)
        add_word(active, W_2)
        add_word(active, W_MINUTY)
    elif extra == 3:
        add_word(active, W_PLUS)
        add_word(active, W_3)
        add_word(active, W_MINUTY)
...
def describe(h, m):
    ...
    text = phrases[m5]
    extras = {1: " + МИНУТА", 2: " + 2 МИНУТЫ", 3: " + 3 МИНУТЫ", 4: " + 4 МИНУТЫ"}
    if extra:
        text += extras[extra]
    return text
...
for page in range(pages):
    ...
    out = f"/home/medivack/Downloads/wordclock_17x16v2_page{page+1}.png"
    sheet.save(out)
