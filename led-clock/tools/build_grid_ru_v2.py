...
COLS = 16
GRID = [
    "ЪДВАДЦАТЬНПЯТЬИЧ",  #  0: ДВАДЦАТЬ _ ПЯТЬ_M
    "РЦДЕСЯТЬГМИНУТЛЗ",  #  1: ДЕСЯТЬ_M _ МИНУТ
    "ЧЕТВЕРТЬПОЛОВИНА",  #  2: ЧЕТВЕРТЬ ПОЛОВИНА
    "ЭБЕЗХДВАДЦАТИПСК",  #  3: БЕЗ _ ДВАДЦАТИ
    "ПЯТИУЧЕТВЕРТИАЯБ",  #  4: ПЯТИ _ ЧЕТВЕРТИ
    "ДЕСЯТИПЕРВОГОТРИ",  #  5: ДЕСЯТИ ПЕРВОГО ТРИ
    "ВТОРОГОПЯТОГОЧАС",  #  6: ВТОРОГО ПЯТОГО ЧАС
    "ТРЕТЬЕГОШЕСТОГОВ",  #  7: ТРЕТЬЕГО ШЕСТОГО
    "ЧЕТВЕРТОГОЧЕТЫРЕ",  #  8: ЧЕТВЕРТОГО ЧЕТЫРЕ
...
def verify():
    ...
    if errors:
        ...
        print(f"\n  ✓ Все ОК. Сетка: {ROWS}×{COLS} = {ROWS * COLS} LED + 4 dots = {ROWS * COLS + 4} LED")
    return errors == 0
